...
moveResultIndex=function moveResultIndex(offset){var _this$results,_this$resultsComponen,_this$resultsComponen2;
...
(_this$resultsComponen2=this.resultsComponent)==null||_this$resultsComponen2.updateHover();
return true;
};_proto.
...
if(this.searchType==='pokemon'&&["sou","subers","smashou","smashubers"].includes(this.format)&&this.dex.species.get(id).tier.startsWith('CAP'))continue;
...
return displayTier(table.overrideTier[id]);
}
if(id.endsWith('totem')&&id.slice(0,-5)in table.overrideTier){
return displayTier(table.overrideTier[id.slice(0,-5)]);
}
...
return displayTier(pokemon.tier);
};_proto2.
...
smashPokemon.length;_i10++){var id=smashPokemon[_i10];results.push(['pokemon',id]);}
}
for(var _id4 in BattlePokedex){
if(smashTiers.has(BattlePokedex[_id4].tier))continue;
switch(_id4){
case'bulbasaur':
...
var species=this.dex.species.get(row[1]);for(var _i12=0;_i12<
...
var badAbilities=[['header',"Unviable Abilities"]];for(var _i14=0,_abilities$sort$map2=
abilities.sort().map(function(abil){return dex.abilities.get(abil);});_i14<_abilities$sort$map2.length;_i14++){var _ability=_abilities$sort$map2[_i14];
...
}[toID((_this$set=this.set)==null?void 0:_this$set.ability)];for(var _i18=0;_i18<
results.length;_i18++){var _item$itemUser;var row=results[_i18];
...
usableMoves.push(['header',signatureMoves.length===1?"Signature move":"Signature moves"]);for(var _i24=0;_i24<
signatureMoves.length;_i24++){var signatureMove=signatureMoves[_i24];
usableMoves.push(['move',signatureMove]);
}
...
sketchMoves.length;_i26++){var _id7=sketchMoves[_i26];
