...
var assetHost=window.Config?Config.routes.assets||Config.routes.client:'play.pokemonshowdown.com';
return prefix+"//"+assetHost+"/";
}();this.
...
var sprite=window.BattleSmashMCSprites[id][shiny?'shiny':'normal'];
var spriteURL=document.location.origin+"/"+sprite.replace(/^\/+/,'');
return"background:transparent url("+spriteURL+") center / contain no-repeat scroll"+_fainted;
}
...
return{
spriteid:spriteid,
spriteDir:'sprites/smashmc',
url:document.location.origin+"/"+
window.BattleSmashMCSprites[id][shiny?'shiny':'normal'].replace(/^\/+/,''),
...
if(data.url){
return"background-image:url("+data.url+");background-position:12px center;background-repeat:no-repeat;background-size:auto 90px";
}
...
j=buf.indexOf('|',i);
var speciesName=buf.substring(i,j);
var species=Dex.species.get(speciesName||name);
