...
export const Dex = new class implements ModdedDex {
	...
	resourcePrefix = (() => {
		...
		const assetHost = window.Config ? Config.routes.assets || Config.routes.client : 'play.pokemonshowdown.com';
	...
	getSpriteData(pokemon: Pokemon | Species | string, isFront: boolean, options: {
		...
		if (window.BattleSmashMCSprites?.[speciesid]) {
			const sprite = window.BattleSmashMCSprites[speciesid];
			const spritePath = sprite[options.shiny ? 'shiny' : 'normal'];
			return {
				...spriteData,
				w: sprite.w || 96,
				h: sprite.h || 96,
	...
	getPokemonIcon(pokemon: string | Pokemon | ServerPokemon | Dex.PokemonSet | null, facingLeft?: boolean) {
		...
		if (window.BattleSmashMCSprites?.[id]) {
			let fainted = ((pokemon as Pokemon | ServerPokemon)?.fainted ?
				`;opacity:.3;filter:grayscale(100%) brightness(.5)` : ``);
			const shiny = !!(pokemon as Pokemon | ServerPokemon)?.shiny;
			const sprite = window.BattleSmashMCSprites[id][shiny ? 'shiny' : 'normal'];
			const spriteURL = `${document.location.origin}/${sprite.replace(/^\/+/, '')}`;
			return `background:transparent url(${spriteURL}) center / contain no-repeat scroll${fainted}`;
		}
